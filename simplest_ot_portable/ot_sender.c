...
void portable_sender_genS(SIMPLEOT_SENDER *s, unsigned char *S_pack) {
  int i;
...
void portable_sender_keygen(SIMPLEOT_SENDER *s, unsigned char *Rs_pack,
                            unsigned char (*keys)[1][HASHBYTES]) {
  if (!portable_sender_keygen_check(s, Rs_pack, keys)) {
    fprintf(stderr, "Error: point decompression failed\n");
    exit(-1);
  }
}

bool portable_sender_keygen_check(SIMPLEOT_SENDER *s, unsigned char *Rs_pack,
                                  unsigned char (*keys)[1][HASHBYTES]) {
  int i;
